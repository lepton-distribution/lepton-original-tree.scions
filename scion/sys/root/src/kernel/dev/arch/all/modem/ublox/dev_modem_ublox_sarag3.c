...
static modem_at_command_t const modem_at_command_list[] = {
   { .command = "ATV1",.response = (const char*)0,.retry=0},
   { .command = "ATE0",.response = (const char*)0,.retry=0 },
   { .command = "AT+CMEE=1",.response = (const char*)0,.retry=0 },
   { .command = "AT+CGREG?",.response = (const char*)0,.retry=0 },
   { .command = "AT+COPS?",.response = (const char*)0,.retry=0 },
   { .command = "AT+CGATT=1",.response = (const char*)0,.retry=0 },
   { .command = "AT+CGREG=1",.response = (const char*)0,.retry=0 },
   { .command = "AT+UPSD=0,0,0",.response = (const char*)0,.retry=0 }, // set IPv4
   { .command = "AT+UPSD=0,1,\"soracom.io\"",.response = (const char*)0,.retry=0 }, //set APN
   { .command = "AT+UPSD=0,2,\"sora\"",.response = (const char*)0,.retry=0 }, //set username
   { .command = "AT+UPSD=0,3,\"sora\"",.response = (const char*)0,.retry=0 }, //set password
   { .command = "AT+UPSDA=0,3",.response = (const char*)0,.retry=0 }, //set Internal PDP context activation
   { .command = "AT+UPSND=0,0",.response = (const char*)0,.retry=0 },  //get IP address
   { .command = "AT+UDCONF=1,1",.response = (const char*)0,.retry=0 },  //set HEX mode for socket read and write operation
   { .command = "AT+ULOCCELL=1",.response = (const char*)0,.retry=0 },  //GSM localisation precision 
   { .command = "AT+ULOC=2,2,2,240,1,1",.response = (const char*)0,.retry=0 }  //GSM localisation
};
...
int ublox_sarag3_at_command_modem_init(void* pv_modem_core_info,void * data) {
   ...
   for (int i = 0; i<(sizeof(modem_at_command_list) / sizeof(modem_at_command_t)); i++) {      
      //
      int retry=modem_at_command_list[i].retry;
      do{
         kernel_printk("trace: %s\r\n", modem_at_command_list[i].command);
         //
         kernel_io_ll_ioctl(desc_r, TCFLSH, TCIFLUSH);
         //
         error= modem_core_parser_send_recv_at_command(p_modem_core_info, modem_at_command_list[i].command, modem_at_command_list[i].response, 0);
         //
         if (error<0 && !retry)
            return -1;
         //
...
static  int dev_modem_ublox_sarag3_ioctl(desc_t desc, int request, va_list ap) {
   switch (request) {
      ...
      default:{
         desc_t desc_serial = ofile_lst[desc].desc_nxt[0];
         if(desc_serial<0)
            return -1;
         //
         if(ofile_lst[desc_serial].pfsop->fdev.fdev_ioctl==(void*)0){
            return -1;
         }
         //
         return ofile_lst[desc_serial].pfsop->fdev.fdev_ioctl(desc_serial,request,ap);
      }
