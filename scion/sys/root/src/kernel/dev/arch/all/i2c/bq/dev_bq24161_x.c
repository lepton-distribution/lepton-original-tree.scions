...
| See:
---------------------------------------------*/
static int bq24161_kernel_reset_i2c(desc_t desc){
   ...
   //i2c reset microcontroller interface
   kernel_io_ll_ioctl(desc_i2c_read,I2C_RESET);
   ...
   for(;;){
      ...
      for(int i=0;i<p_dev_bq24161_info->registers_list_size;i++){
         if(bq24161_write_register(desc,
            ...
            kernel_printk("bq24161: write error on register 0x%02x \r\n", p_dev_bq24161_info->p_registers_list[i].address);
            bq24161_kernel_reset_i2c(desc);
            kernel_printk("bq24161: kernel reset i2c interface!\r\n");
