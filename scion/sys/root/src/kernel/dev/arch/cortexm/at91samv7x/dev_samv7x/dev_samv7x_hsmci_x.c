...
int dev_samv71x_hsmci_x_read(desc_t desc, char* buf,int size){
   ...
   */
   alignedAddr = (uint32_t)samv71x_hsmci_info->p_rw_aligned_dma_buffer & ~3;
   SCB_CleanDCache_by_Addr((uint32_t*)alignedAddr, size + ((uint32_t)samv71x_hsmci_info->p_rw_aligned_dma_buffer - alignedAddr));
   //
