...
//#if 0 //to do: migration in progress to hal cube mx 1.16
      ...
      sdio_info->sd_state = HAL_SD_Init(&sdio_info->hsd);
      __disable_interrupt_section_out();
      #ifdef BUS_4BITS
         /* Configure SD Bus width */
         if (sdio_info->sd_state == MSD_OK){
         /* Enable wide operation */
            if (HAL_SD_ConfigWideBusOperation(&sdio_info->hsd, SDIO_BUS_WIDE_4B) != SD_OK){
...
int dev_stm32f4xx_sdio_read(desc_t desc, char* buf,int size){
   ...
   //
   if(HAL_SD_ReadBlocks(&sdio_info->hsd, (uint8_t*)buf, ofile_lst[desc].offset/sdio_info->BlockSize, size/sdio_info->BlockSize,10000) != SD_OK){
      sdio_info->sd_state = MSD_ERROR;
      return -1;
   }  
   //
...
int dev_stm32f4xx_sdio_write(desc_t desc, const char* buf,int size){
   ...
   //
   if(HAL_SD_WriteBlocks(&sdio_info->hsd, (uint8_t*)buf, ofile_lst[desc].offset/sdio_info->BlockSize, size/sdio_info->BlockSize,10000) != SD_OK){
      sdio_info->sd_state = MSD_ERROR;
      return -1;
   }
   
   //wait end of previous operation 
   ...
   do
   {
      state_return = HAL_SD_GetState(&sdio_info->hsd);
      timeout++;
   }while ((HAL_SD_STATE_BUSY == state_return) && (SD_WAIT_TIMEOUT > timeout)); 
   //
