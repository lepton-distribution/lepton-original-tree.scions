   ...
   if(sdio_info->desc_r<0 && sdio_info->desc_w<0) {
      ...
      sdio_info->sd_state = HAL_SD_Init(&sdio_info->hsd);
         ...
         /* Enable wide operation */
            if (HAL_SD_ConfigWideBusOperation(&sdio_info->hsd, SDIO_BUS_WIDE_4B) != MSD_OK){
...
int dev_stm32f4xx_sdio_read(desc_t desc, char* buf,int size){
   ...
   //
   if(HAL_SD_ReadBlocks(&sdio_info->hsd, (uint8_t*)buf, ofile_lst[desc].offset, size/sdio_info->BlockSize,SD_DATATIMEOUT) != MSD_OK){
      sdio_info->sd_state = MSD_ERROR;
      return -1;
   }  
   //
...
int dev_stm32f4xx_sdio_write(desc_t desc, const char* buf,int size){
   ...
   //
   if(HAL_SD_WriteBlocks(&sdio_info->hsd, (uint8_t*)buf, ofile_lst[desc].offset,size/sdio_info->BlockSize,SD_DATATIMEOUT) != MSD_OK){
      sdio_info->sd_state = MSD_ERROR;
      return -1;
   }
...
int dev_stm32f4xx_sdio_ioctl(desc_t desc,int request,va_list ap){
   ...
   switch(request) {
      case HDGETSZ: {
         ...
         HAL_SD_GetCardInfo(&sdio_info->hsd, &sdio_info->SDCardInfo);
         //
         *hdsz_p= sdio_info->SDCardInfo.LogBlockNbr*sdio_info->SDCardInfo.LogBlockSize;
      }
      break;
      ...
      case HDGETSCTRSZ: {
         ...
         if(!sector_sz)
            return -1;
         //
         *sector_sz = sdio_info->SDCardInfo.LogBlockSize;
      }
      break;
      ...
      case HDCLRDSK: {
         ...
         unsigned long EndAddr =  (sdio_info->SDCardInfo.LogBlockNbr-1)*sdio_info->SDCardInfo.LogBlockSize;
         
         if(HAL_SD_Erase(&sdio_info->hsd, StartAddr, EndAddr) != MSD_OK){
            sdio_info->sd_state = MSD_ERROR;
         }else{
            sdio_info->sd_state = MSD_OK;
         }
